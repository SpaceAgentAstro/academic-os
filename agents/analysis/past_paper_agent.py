...
def detect_paper_type(pdf_path: Path, sample: str | None = None) -> str:
    """Identify whether a PDF is a question_paper, mark_scheme, or examiner_report.

    Pass ``sample`` (the first couple of pages' text) when the caller has already
    parsed the PDF, to avoid re-parsing it (RT-008).
    """
...
def extract_metadata(pdf_path: Path) -> PaperMetadata:
    ...
    paper_type = detect_paper_type(pdf_path, sample=sample)
